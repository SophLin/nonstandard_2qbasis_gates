# a copy of the visualize module of the weylchamber package, in case we need it
...
class WeylChamber():
    ...
    def __init__(self):
        ...
        self.labels = [
            ('I_1', self.A1 + np.array((0.03, 0.00 , -0.01)) ), # A_1
            ('iSWAP' , self.A2 + np.array((0.01, 0, -0.01)) ), # A_2
            ('SWAP' , self.A3 + np.array((-0.01, 0, 0.01)) ), # A_3
            ('I_0'   , self.O  + np.array((-0.025,  0.0, 0.02)) ), # O
            ('CZ'   , self.L  + np.array((-0.05, 0, -0.05)) ), # L
            ('\sqrt{iSWAP}'   , self.M  + np.array((0.05, -0.01, 0)) ), # M
            ('\sqrt{SWAP}^\dagger'   , self.N  + np.array((-0.075, 0, 0.009)) ),# N
            ('\sqrt{SWAP}'   , self.P  + np.array((-0.13, -0.02, 0.008)) ),# P
            ('\sqrt{iSWAP}'   , self.Q  + np.array((-0.03, 0.01, 0.02)) )# Q
        ]
        ...
        self._scatter = []
        self.textmode = 0 # 0: showing all labels, 1: showing only the vertices, 2: showing no label
        self.addlines = []
    ...
    def render(self, ax):
        ...
        # labels
        if self.textmode == 0:
            for label, labelcoords in self.labels:
                c1, c2, c3 = labelcoords
                if self.tex_labels:
                    ax.text(c1, c2, c3, "$%s$" % label, **self.label_properties)
                else:
                    ax.text(c1, c2, c3, label, **self.label_properties)
        elif self.textmode == 1:
            for label, labelcoords in self.labels[:4]:
                c1, c2, c3 = labelcoords
                if self.tex_labels:
                    ax.text(c1, c2, c3, "$%s$" % label, **self.label_properties)
                else:
                    ax.text(c1, c2, c3, label, **self.label_properties)
        elif self.textmode == 2:
            pass
        # foreground lines
        for P1, P2, fg in self.weyl_edges:
            if fg:
        ...
        for origin, end, lineargs in self.addlines:
            self._draw_line(ax, origin, end, **lineargs)
        ax.axis('off') # I added this
    ...
    # I added the funciton
    # origin and end should be 3-tuples
    def draw_line(self,origin,end,props):
        self.addlines.append((origin, end, props))
